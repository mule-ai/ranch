...
| `ranch_ask` | `question`, `choices[]`, `suggested?`, `multi?`, `free_text?` | Blocks the agent's turn until the human answers a multiple-choice question (with optional suggested option and/or a blank free-text option; `multi` = select-many). Broadcast to every attached client (`AgentAskRequest`); first answer wins (`AgentAskAnswer`); agent polls `AgentAskStatus` until answered — there is no timeout (the user may take hours); a daemon restart resolves a blocked ask as "no answer (ask lost)". Mobile: pushes a local notification (settings toggle). |
